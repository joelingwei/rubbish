...
Page({
  ...
  onShow: function () {
    var that = this;
    //获取缓存中的城市id
    var cid = swan.getStorageSync('cid');
    //如果城市id不存在则要发起定位授权请求
    if (!cid) {
      swan.getSetting({
        success(res) {
                ...
                utils.getLocal(res.latitude, res.longitude, function (events) {
                  ...
                  utils.rubbishCity(events.substring(0, events.length - 1), 'name', function (e) {
                    if (e.rubbish_list == false) {
                      if (swan.getStorageSync('city_fenlei') == false) {
                        swan.setStorageSync('city_fenlei', that.data.rubbish_list);
                      }
                      swan.showToast({
  ...
  //停止录音
  handleRecordStop() {
    swan.hideToast(); //结束录音、隐藏Toast提示框
    this.setData({
      audios: {
        'img': 'voice',
        'name': '语音查询'
