...
Page({
  ...
  onShow: function() {
    var fenlei_huan =my.getStorageSync({ key: 'city_fenlei' })
    if (!fenlei_huan){
  ...
  downLoadQuestion: function() {
    ...
    for (var i = 0; i < this.data.fenlei_detail.length; i++) {
      this.data.fenlei_detail[i].name = fenlei_huan.data['list'][i].name
    }
  ...
  bindImage: function() {
    var that = this;
    const ctx = my.createCanvasContext('share');
    //背景图
  ...
  onShareAppMessage(res) {
    ...
    return {
      ...
      bgImgUrl: '/images/examp_share.jpg'
    };
  }
})